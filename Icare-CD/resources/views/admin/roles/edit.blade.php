            ...
            <div class="px-4 py-5 sm:p-6">
                ...
                @if($role->is_system_role)
                    <div class="mb-6 bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-lg">
                        <p class="text-sm">⚠️ This is a system role. You can modify permissions but cannot change the name or delete it.</p>
                    </div>
                @endif
                ...
                <form action="{{ route('admin.roles.update', $role) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required
                            ...
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('name') border-red-300 @enderror {{ $role->is_system_role ? 'bg-gray-100' : '' }}">
                        @error('name')
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('description') border-red-300 @enderror">{{ old('description', $role->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
