    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-slate-900">Create Test Category</h1>
                    <p class="mt-0.5 text-sm text-slate-600">Add a new category for grouping test sets</p>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('admin.test-categories.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label for="slug" class="block text-sm font-medium text-slate-700 mb-1.5">
                            Slug <span class="text-slate-400 text-xs">(auto-generated if empty)</span>
                        </label>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3" placeholder="Optional. Describe what this category is for..."
                                  class="w-full px-3 py-2 rounded-lg border border-slate-300 bg-white text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none">{{ old('description') }}</textarea>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        ...
                        @foreach($icons as $iconKey => $iconLabel)
                            <label class="cursor-pointer">
                                <input type="radio" name="icon" value="{{ $iconKey }}" class="sr-only peer"
                                       {{ old('icon') == $iconKey ? 'checked' : '' }} required>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div id="color-preview" class="ml-2 px-3 py-1.5 rounded-md text-xs font-medium" style="background-color: {{ old('color', '#3B82F6') }}20; color: {{ old('color', '#3B82F6') }};">
                            Preview
                        </div>
                    </div>
