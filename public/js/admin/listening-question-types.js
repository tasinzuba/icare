...
window.ListeningQuestionTypes = {
    ...
    loadFillBlanksData(questionData) {
        ...
        if (questionData.section_specific_data) {
            try {
                ...
                console.log('Parsed section specific data:', data);

                // Try different possible data structures
                if (data && data.blanks) {
                    this.blankAnswers = data.blanks;
                    console.log('Loaded blank answers from blanks:', this.blankAnswers);
                } else if (data && data.blank_answers) {
                    this.blankAnswers = data.blank_answers;
                    console.log('Loaded blank answers from blank_answers:', this.blankAnswers);
                }
            } catch (e) {
    ...
    loadSingleChoiceData(questionData) {
        ...
        console.log('listening_data type:', typeof questionData.section_specific_data);
        console.log('listening_data value:', questionData.section_specific_data);
        ...
        // Parse options from questionData.options or section_specific_data
        let options = [];
        let correctOption = 0;

        // First try to get from questionData.options (from database options relationship)
        if (questionData.options && Array.isArray(questionData.options) && questionData.options.length > 0) {
            ...
            if (correctOpt) {
                correctOption = options.indexOf(correctOpt);
            }
            console.log('Loaded options from questionData.options:', options);
            console.log('Correct option index:', correctOption);
        }
        // Fallback to section_specific_data
        else if (questionData.section_specific_data) {
            try {
                const data = typeof questionData.section_specific_data === 'string'
                    ? JSON.parse(questionData.section_specific_data)
                    : questionData.section_specific_data;
                ...
                console.log('Found options from section_specific_data:', options);
                console.log('Correct option index:', correctOption);
            } catch (e) {
    ...
    loadMultipleChoiceData(questionData) {
        ...
        if (questionData.options && Array.isArray(questionData.options) && questionData.options.length > 0) {
            ...
            console.log('Loaded options from questionData.options:', options);
            console.log('Correct options indices:', correctOptions);
        }
        // Fallback to section_specific_data
        else if (questionData.section_specific_data) {
            try {
    ...
    loadDropdownData(questionData) {
        ...
        // Parse dropdown data from section_specific_data
        if (questionData.section_specific_data) {
            try {
                const data = typeof questionData.section_specific_data === 'string'
                    ? JSON.parse(questionData.section_specific_data)
                ...
                console.log('Parsed section specific data for dropdown:', data);

                // Try different possible data structures
                if (data && data.dropdowns) {
                    this.dropdownData = data.dropdowns;
                    console.log('Loaded dropdown data from dropdowns:', this.dropdownData);
                } else if (data && data.dropdown_options && data.dropdown_correct) {
                    // Convert to expected format
                    this.dropdownData = {
                        options: data.dropdown_options,
                        correct: data.dropdown_correct
                    };
                    console.log('Loaded dropdown data from dropdown_options/dropdown_correct:', this.dropdownData);
                }
            } catch (e) {
                console.error('Error parsing section specific data:', e);
            }
