<x-admin-layout>
    <x-slot:title>Add Avatar Teacher</x-slot>
    ...
    <div class="mb-8">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-8 shadow-xl">
            ...
            <div class="relative flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-white">Create Avatar Teacher</h1>
                    <p class="mt-2 text-indigo-100">Add a new AI avatar teacher for Speaking tests</p>
                </div>
    ...
    <!-- Form Container -->
    <form action="{{ route('admin.avatar-teachers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    <label for="photo" class="block text-sm font-semibold text-gray-700 mb-2">
                        Teacher Photo <span class="text-red-500">*</span>
                    </label>
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl hover:border-indigo-400 transition-colors" id="photo-drop-zone">
                        <div class="space-y-1 text-center">
                            ...
                            <p class="text-xs text-gray-500">PNG, JPG up to 5MB. Front-facing, neutral expression recommended.</p>
                        </div>
                    </div>
                    @error('photo')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="accent"
                            ...
                            <option value="british" {{ old('accent') === 'british' ? 'selected' : '' }}>British</option>
                            <option value="american" {{ old('accent') === 'american' ? 'selected' : '' }}>American</option>
                            <option value="australian" {{ old('accent') === 'australian' ? 'selected' : '' }}>Australian</option>
                            <option value="neutral" {{ old('accent') === 'neutral' ? 'selected' : '' }}>Neutral</option>
                        </select>
                        @error('accent')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow-lg border border-gray-100">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                                ...
                                <optgroup label="Your Custom Voices">
                                    @foreach($groupedVoices['custom'] as $voice)
                                        <option value="{{ $voice['voice_id'] }}"
                                                ...
                                                {{ old('elevenlabs_voice_id') === $voice['voice_id'] ? 'selected' : '' }}>
                    ...
                    <input type="hidden" name="voice_name" id="voice_name" value="{{ old('voice_name') }}">
        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow-lg border border-gray-100">
            <div class="bg-gradient-to-r from-green-50 to-emerald-50 px-6 py-5 border-b border-gray-100">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-sm text-gray-600">Additional configuration options</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-sm font-semibold text-gray-900">Set as Default Teacher</h4>
    ...
    // Initialize voice name if already selected
