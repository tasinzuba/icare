<x-admin-layout>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    <div class="space-y-6">
                        ...
                        <div id="reading-drag-drop-panel" class="mb-4 border border-indigo-200 rounded-lg bg-indigo-50 p-4" style="display: none;">
                            ...
                            <p class="text-xs text-gray-600 mb-3">In the Question box above, insert <code>[DRAG_1]</code>, <code>[DRAG_2]</code>… gaps. Add the word options students drag from, then pick the correct word for each gap.</p>
                            <div class="mb-4">
    ...
    function handleReadingQuestionTypeChange() {
        ...
        if (type === 'passage') {
            ...
            if (!window.isEditMode && document.querySelectorAll('.reading-drag-opt-input').length === 0) {
    ...
    function loadExistingData() {
        ...
        // Load existing drag & drop data
        if (existingType === 'drag_drop' && window.questionData.section_specific_data) {
            const ssd = window.questionData.section_specific_data;
            const optsBox = document.getElementById('reading-drag-options');
            if (optsBox) optsBox.innerHTML = '';
            (ssd.draggable_options || []).forEach(function (o) {
                if (window.readingAddDragOption) window.readingAddDragOption(o);
            });
            if (window.readingRefreshDragZones) window.readingRefreshDragZones();
            setTimeout(function () {
                (ssd.drop_zones || []).forEach(function (z) {
                    const sel = document.querySelector('select[name="drag_zones[' + z.zone_number + '][answer]"]');
                    if (sel) sel.value = (z.answer || z.correct_answer || '');
                });
            }, 250);
        }
    }
